...
if ~usejava('jvm')
    ...
    'Visible','on',...
    'HandleVisibility','on');
    hLogoPos = get(hLogo,'Position');
    set(hLogo,'Position',[hLogoPos(1) hLogoPos(2) size(I,2) size(I,1)])
    AxPos = [1 1 size(I,2) size(I,1)];
    AxLogo = axes('Units','pixels','Position',AxPos);
    imshow(I,'Parent',AxLogo);
    uiwait(hLogo,time/1000);
    close(hLogo);
else
